...
function draw_design(js_enroll_data, div_id, colors) {
    ...
    var included_ls = [];
    var excluded_ls = [];
    for (var i = 0; i < js_enroll_data['participants'].length; i++) {
        // split by negated
        if (js_enroll_data['participants'][i]['negation'] === true) {
            //excluded
            excluded_ls.push(js_enroll_data['participants'][i]['term']);
        } else if (js_enroll_data['participants'][i]['negation'] === false) {
            //includes
            included_ls.push(js_enroll_data['participants'][i]['term']);
        }
    }
    ...
    if (div_id === "enrollment") {
        ...
        if (js_enroll_data['participants'].length === 0 ) {
            container_enroll.innerHTML = "<p style='display: flex; justify-content: center; align-items: center; height: 100%;'>No participants found in study</p>";
        } else {
        ...
        if (js_enroll_data['participants'].length === 0 ) {
            container_enroll_popup.innerHTML = "<p style='display: flex; justify-content: center; align-items: center; height: 100%;'>No participants found in study</p>";
        } else {
            network_enroll = new vis.Network(container_enroll_popup, data_enroll, options);
        }
    }
}
