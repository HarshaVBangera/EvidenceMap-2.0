...
class EvidenceBaseNegationDriver(ABC):
    available_drivers = {}
    default_driver = "V0"
    ...
    @abstractmethod
    def __init__(self, model_driver: EvidenceBaseModelDriver, config):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if "_identifier" in cls.__dict__:
            EvidenceBaseNegationDriver.available_drivers[cls._identifier] = cls
        else:
            raise ValueError(f"Class {cls.__name__} does not define an identifier")

    @abstractmethod
    def detectNegations(self, model_predictions):
        pass
    ...
    def import_subclasses(cls):
        ...
        for child_dir_name in os.listdir(parent_directory):
            ...
            if os.path.isdir(child_dir_path):
                module_path = f"Negations.{child_dir_name}.NegationDriver"
                try:
...
EvidenceBaseNegationDriver.import_subclasses()
