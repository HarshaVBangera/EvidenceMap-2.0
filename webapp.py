...
def display_by_pages(query, solr_query, Participant_list, Intervention_list, Outcome_list, option="best"):
    ...
    summary_data = collective_visualize().get_json()
    results_summary = summarizer.generate_summary(summary_data, display_results)
...
def collective_visualize(no_args=False):
    ...
    for doc in docs:
        original_doc = solr_unflatten(doc)
        if 'study_results' in original_doc:
            total_results_before += len(original_doc['study_results'])
        print(f"Total nodes before clustering: {total_results_before}")        
        ...
        for data in combined_level3_data:
            if 'study_results' in data:
                total_results_after += len(data['study_results'])
        print(f"Total nodes after clustering: {total_results_after}")
